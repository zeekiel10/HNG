...
## 📖 Project Overview
This project analyzes polling unit results from Ebonyi State, Nigeria to detect unusual voting patterns (outliers).  
By combining geolocation, clustering, and statistical checks, we identify polling units whose results differ significantly from their neighbors.  
...
- Compared each polling unit’s party vote counts (APC, LP, PDP, NNPP) against the average of its neighbors.  
...
## 🧾 Conclusion
This project demonstrates how **data science can support election transparency**.  
By combining geolocation with statistical analysis, we can quickly highlight polling units that deserve closer scrutiny.
